...
import { useStudentLogin } from "@/lib/react-query/auth/student";
...
const StudentSignInForm = () => {
  ...
  const { mutateAsync: studentLogin, isPending } = useStudentLogin();
  ...
  const onSubmit = async ({
    ...
    try {
      const { data, message, status } = await studentLogin({
        email,
        password,
      });
      // ! Login Failed
  ...
  return (
    ...
    >
      <div className="w-full max-w-[520px] p-4 md:p-8">
        <h2 className="text-3xl font-bold text-main mt-10 uppercase">LOGIN</h2>
        ...
        <p className="text-left font-sora font-normal text-base text-black-blue mt-4">
          ...
          <Link to="/sign-up" className="text-main hover:underline ms-1">
            Create account
          </Link>
        </p>
      </div>
